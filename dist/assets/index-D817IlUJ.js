(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e;function t(){e=L.map(`map`,{center:[43.55,3.45],zoom:10,zoomControl:!1,renderer:L.canvas()}),L.control.zoom({position:`topright`}).addTo(e);let t=L.tileLayer(`https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png`,{maxZoom:19,attribution:`© OpenStreetMap`}),n=L.tileLayer(`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}`,{attribution:`Esri Satellite`}),r={"🗺️ Carte Villes & Villages (CartoDB)":L.tileLayer(`https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png`,{attribution:`© CartoDB Voyager`}).addTo(e),"📐 Parcellaire Cadastre (IGN)":L.tileLayer(`https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=CADASTRALPARCELS.PARCELLAIRE_EXPRESS&STYLE=normal&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png`,{attribution:`© IGN Cadastre France`,opacity:.7}),"🛰️ Vue Satellite Haute Résolution":n,"🌐 OpenStreetMap Standard":t};return L.control.layers(r,null,{position:`topright`}).addTo(e),e.createPane(`polygonsPane`),e.getPane(`polygonsPane`).style.zIndex=400,e.createPane(`fossilsPane`),e.getPane(`fossilsPane`).style.zIndex=650,e}function n(t,n,r=14){e&&e.flyTo([t,n],r,{duration:1.5})}function r(e){if(e.key===`Enter`){let t=e.target.value.toLowerCase().trim();if(!t)return;fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(t+`, Hérault, France`)}`).then(e=>e.json()).then(e=>{e&&e.length>0?n(parseFloat(e[0].lat),parseFloat(e[0].lon),13):alert(`Lieu introuvable dans l'Hérault.`)})}}function i(e,t){let n=document.getElementById(`wiki-box-${t}`);if(!n)return;n.style.display=`block`,n.innerHTML=`<i class="fa-solid fa-spinner fa-spin"></i> Recherche scientifique...`;let r=e.replace(/["'()]/g,``).replace(/\b(sp|cf|var|subsp)\b\.?/gi,``).trim(),i=`
  ...
  `;fetch(`https://fr.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(r)}&format=json&origin=*`).then(e=>e.json()).then(e=>{if(e&&e.query&&e.query.search&&e.query.search.length>0){let t=e.query.search[0].title;return fetch(`https://fr.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(t)}`).then(e=>e.json())}else return fetch(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(r)}&format=json&origin=*`).then(e=>e.json()).then(e=>e&&e.query&&e.query.search&&e.query.search.length>0?fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(e.query.search[0].title)}`).then(e=>e.json()):null)}).then(e=>{if(e&&e.extract){let t=``;e.thumbnail&&e.thumbnail.source&&(t=`<img src="${e.thumbnail.source}" style="width:100%; max-height:120px; object-fit:cover; border-radius:6px; margin-bottom:6px;">`),n.innerHTML=`
        ...
        ${i}
      `})}var a=[],o,s=new Set([`dinosaurs_reptiles`,`molluscs`,`mammals`,`plants`,`arthropods`,`fish`,`others`]),c=new Set([`MNHN`,`PBDB`,`BRGM`]),l=``;function u(){o=L.layerGroup().addTo(e),e&&e.on(`zoomend`,()=>{h()}),window.loadWikiPreview=i}function d(e){a=e,h()}function f(e,t){t?s.add(e):s.delete(e),h()}function p(e,t){t?c.add(e):c.delete(e),h()}function m(e){l=e,h()}function h(){if(!o)return;o.clearLayers();let t=0,n={dinosaurs_reptiles:{icon:`fa-dragon`,color:`#ef4444`,label:`Dinosaures / Reptiles`},molluscs:{icon:`fa-note-sticky`,color:`#0284c7`,label:`Mollusques / Ammonites`},plants:{icon:`fa-leaf`,color:`#16a34a`,label:`Plantes / Walchia`},arthropods:{icon:`fa-bug`,color:`#9333ea`,label:`Trilobites / Arthropodes`},mammals:{icon:`fa-bone`,color:`#d97706`,label:`Mammifères`},fish:{icon:`fa-fish`,color:`#0d9488`,label:`Poissons`},others:{icon:`fa-circle-dot`,color:`#64748b`,label:`Autres`}};a.forEach(r=>{if(!s.has(r.category_id))return;let i=r.source||`PBDB`;if(!c.has(i))return;if(l){let e=(r.period||``).toLowerCase();if(l===`permian`&&!e.includes(`permian`)||l===`jurassic`&&!e.includes(`jurassic`)||l===`cretaceous`&&!e.includes(`cretaceous`)||l===`ordovician_devonian`&&!e.includes(`ordovician`)&&!e.includes(`devonian`)||l===`cenozoic`&&!e.includes(`neogene`)&&!e.includes(`pliocene`)&&!e.includes(`eocene`)&&!e.includes(`miocene`)&&!e.includes(`paleogene`))return}t++;let a=n[r.category_id]||n.others,u=a.icon;r.category_id===`molluscs`&&(u=`fa-ring`);let d=e?e.getZoom():10,f=d>=13?7:d>=10?5:3.5,p=L.circleMarker([r.lat,r.lng],{radius:f,fillColor:a.color,color:`#ffffff`,weight:d>=11?1.5:.8,fillOpacity:.85}),m=`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(r.name+` fossil`)}`,h=`https://www.google.com/search?q=${encodeURIComponent(r.name+` fossile`)}`,g=r.name.replace(/[^a-zA-Z0-9 ]/g,``).trim(),_=r.precision_gps||`🎯 GPS Précis`,v=r.precision_code===`high`?`#10b981`:`#f59e0b`,y={MNHN:`<span style="background:rgba(168,85,247,0.2); color:#c084fc; border:1px solid rgba(168,85,247,0.5); padding:1px 6px; border-radius:4px; font-weight:700; font-size:0.75rem;">🏛️ Collection Muséum (MNHN Paris)</span>`,BRGM:`<span style="background:rgba(56,189,248,0.2); color:#38bdf8; border:1px solid rgba(56,189,248,0.5); padding:1px 6px; border-radius:4px; font-weight:700; font-size:0.75rem;">📐 Carte Géologique BRGM</span>`,PBDB:`<span style="background:rgba(34,197,94,0.2); color:#4ade80; border:1px solid rgba(34,197,94,0.5); padding:1px 6px; border-radius:4px; font-weight:700; font-size:0.75rem;">🌐 Base Scientifique PBDB</span>`},b=y[i]||y.PBDB,x=r.mnhn_catalog?`<br><b>N° Spécimen Muséum :</b> <span style="color:#c084fc; font-weight:600;">${r.mnhn_catalog}</span>`:``,S=`
      <div class="popup-title">${r.name}</div>
      <div style="margin-bottom:6px;">${b}</div>
      <div class="popup-meta">
        <b>Type :</b> <span style="color:${a.color}; font-weight:600;"><i class="fa-solid ${u}"></i> ${r.category_name}</span><br>
        <b>Précision Spatiale :</b> <span style="color:${v}; font-weight:600;">${_}</span>${x}<br>
        <b>Période :</b> ${r.period} (${r.max_ma?r.max_ma+` Ma`:`N/A`})<br>
        <b>Formation :</b> ${r.formation||`Non spécifiée`}<br>
        <b>Phylum / Classe :</b> ${r.phylum} / ${r.class_name}<br>
        <b>Coordonnées GPS :</b> ${r.lat.toFixed(4)}, ${r.lng.toFixed(4)}
      </div>

      <div id="wiki-box-${r.id}" style="margin-top:8px; font-size:0.78rem; color:#cbd5e1; background:rgba(0,0,0,0.3); padding:6px 8px; border-radius:6px; border:1px solid rgba(255,255,255,0.08); display:none;"></div>

      <div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:8px;">
        <a href="${m}" target="_blank" class="popup-tag" style="background:#0284c7; color:#fff; border-color:#0284c7;">
          🖼️ Photos Google
        </a>
        <a href="${h}" target="_blank" class="popup-tag" style="background:#475569; color:#fff; border-color:#475569;">
          🔍 Recherche Web
        </a>
        <button onclick="loadWikiPreview('${g}', ${r.id})" class="popup-tag" style="background:#15803d; color:#fff; border-color:#15803d; cursor:pointer;">
          📖 Aperçu Wikipédia
        </button>
        <a href="https://paleobiodb.org/classic/basicCollectionSearch?collection_no=${r.id}" target="_blank" class="popup-tag" style="background:#1e293b; color:#94a3b8; border-color:#334155;">
          📄 PBDB #${r.id}
        </a>
      </div>
    `;p.bindPopup(S),o.addLayer(p)});let r=document.getElementById(`stat-fossils`);r&&(r.innerText=t.toLocaleString(`fr-FR`))}var g=null,_=null,v=`score`,y=45,b=!1;function x(e){g=e,T()}function S(e){v=e;let t=document.getElementById(`mapModeLegend`);t&&(e===`period`?t.innerHTML=`<b>🎨 Carte Stratigraphique BRGM :</b><br>
  ...
  `).addTo(e)}function j(t){O&&e&&(t?e.addLayer(O):e.removeLayer(O))}function M(t){E&&e&&(t?e.addLayer(E):e.removeLayer(E))}function N(t){D&&e&&(t?e.addLayer(D):e.removeLayer(D))}var P=null,F=!1;function ee(){e&&(e.on(`click`,function(e){if(window.innerWidth<=768){let e=document.getElementById(`sidebar`);e&&e.classList.remove(`mobile-open`)}if(F){F=!1;let t=e.latlng.lat,n=e.latlng.lng;R(t,n,e.latlng)}}),e.on(`contextmenu`,function(e){R(e.latlng.lat,e.latlng.lng,e.latlng)}))}function I(){if(!navigator.geolocation){alert(`La géolocalisation n'est pas supportée par votre navigateur.`);return}navigator.geolocation.getCurrentPosition(t=>{let n=t.coords.latitude,r=t.coords.longitude;P&&e&&e.removeLayer(P),P=L.marker([n,r],{icon:L.divIcon({className:`custom-user-location`,html:`<div style="background:#2563eb; color:#fff; width:26px; height:26px; border-radius:50%; border:3px solid #fff; box-shadow:0 0 15px #2563eb; display:flex; align-items:center; justify-content:center; font-size:12px;"><i class="fa-solid fa-crosshairs"></i></div>`,iconSize:[26,26],iconAnchor:[13,13]})}).bindPopup(`
      ...
      `).addTo(e),e.flyTo([n,r],14,{duration:1.5}),P.openPopup()},e=>{alert(`Impossible d'obtenir votre position GPS. Astuce: Vous pouvez faire un appui long sur la carte pour placer un point.`)},{enableHighAccuracy:!0})}function R(t,n,r){L.popup().setLatLng(r).setContent(`
  ...
  `).openOn(e)}function z(){let t=e.getCenter(),n=t.lat,r=t.lng,i=`<?xml version="1.0" encoding="UTF-8"?>
...
</gpx>`;let o=new Blob([i],{type:`application/gpx+xml`}),s=document.createElement(`a`);s.href=URL.createObjectURL(o),s.download=`fossiles_herault_${n.toFixed(3)}_${r.toFixed(3)}.gpx`,s.click()}function B(){let t=e.getCenter(),n=t.lat,r=t.lng,i=`<?xml version="1.0" encoding="UTF-8"?>
<kml xmlns="http://www.opengis.net/kml/2.2">
...
</kml>`;let o=new Blob([i],{type:`application/vnd.google-earth.kml+xml`}),s=document.createElement(`a`);s.href=URL.createObjectURL(o),s.download=`fossiles_herault_${n.toFixed(3)}_${r.toFixed(3)}.kml`,s.click()}function V(){let t=e.getCenter();H(t.lat,t.lng)}function H(e,t){let n=window.open(``,`_blank`);n.document.write(`
    <html>
    <head>
  ...
  `),n.document.close()}var U=null;function W(){U=L.layerGroup().addTo(e),J()}function G(t=null,n=null){let r=document.getElementById(`personalFindingModal`);r&&(r.style.display=`flex`);let i=e.getCenter(),a=document.getElementById(`pf-lat`),o=document.getElementById(`pf-lng`);a&&(a.value=t?t.toFixed(5):i.lat.toFixed(5)),o&&(o.value=n?n.toFixed(5):i.lng.toFixed(5))}function K(){let e=document.getElementById(`personalFindingModal`);e&&(e.style.display=`none`)}function q(t){t.preventDefault();let n=document.getElementById(`pf-name`).value,r=document.getElementById(`pf-category`).value,i=parseFloat(document.getElementById(`pf-lat`).value),a=parseFloat(document.getElementById(`pf-lng`).value),o=document.getElementById(`pf-notes`).value,s={id:Date.now(),name:n,category:r,lat:i,lng:a,notes:o,date:new Date().toLocaleDateString(`fr-FR`)},c=JSON.parse(localStorage.getItem(`my_fossil_findings`)||`[]`);c.push(s),localStorage.setItem(`my_fossil_findings`,JSON.stringify(c)),K(),document.getElementById(`pf-name`).value=``,document.getElementById(`pf-notes`).value=``,J(),e.flyTo([i,a],14)}function J(){U&&(U.clearLayers(),JSON.parse(localStorage.getItem(`my_fossil_findings`)||`[]`).forEach(e=>{let t=L.marker([e.lat,e.lng],{pane:`fossilsPane`,icon:L.divIcon({className:`custom-personal-finding`,html:`<div style="background:#a855f7; color:#fff; width:26px; height:26px; border-radius:50%; border:2px solid #fff; box-shadow:0 0 12px #a855f7; display:flex; align-items:center; justify-content:center; font-size:12px;"><i class="fa-solid fa-star"></i></div>`,iconSize:[26,26],iconAnchor:[13,13]})}).bindPopup(`
    ...
    `),t.keySites&&t.keySites.length>0){let e=t.keySites[0];n(e.lat,e.lng,e.zoom)}}window.selectPaleoEra=oe,window.locateUserOnMap=I,window.openAddPersonalFindingModal=G,window.closeAddPersonalFindingModal=K,window.savePersonalFinding=q,window.deletePersonalFinding=Y,window.downloadGPXExport=z,window.downloadKMLExport=B,window.generateCurrentLocationReport=V,window.generateCustomLocationReport=H,window.toggleSidebar=X,window.toggleMobileSidebar=Z,window.toggleMobileMapMode=Q,window.switchMapColorMode=S,window.filterByPeriod=m,window.toggleSourceFilter=p,window.toggleReservesLayer=j,window.toggleRiversLayer=M,window.toggleQuarriesLayer=N,window.toggleSlopeFilter=w,window.updateScoreFilter=C,window.toggleCategory=f,window.openGearModal=te,window.closeGearModal=ne,window.openGuideModal=re,window.closeGuideModal=ie,window.handleSearch=r,window.loadWikiPreview=i;var $={};window.changeDepartment=function(e){let t=$[e];t&&(n(t.center[0],t.center[1],10),fetch(`processed/${e}/fossils.json`).then(e=>{if(!e.ok)throw Error(`Dataset not pre-packaged`);return e.json()}).then(e=>d(e)).catch(()=>{let e=t.bounds,n=`https://paleobiodb.org/data1.2/occs/list.json?lngmin=${e[2]}&lngmax=${e[3]}&latmin=${e[0]}&latmax=${e[1]}&show=coords,classext,strata`;fetch(n).then(e=>e.json()).then(e=>{e&&e.records&&d(e.records.map(e=>({id:e.oid,name:e.tno||e.mno||`Fossile PBDB`,lat:e.lat,lng:e.lng,phylum:e.phl||`PBDB`,class_name:e.cll||`Paleontology`,period:e.dpt||e.eon||`Mésozoïque`,formation:e.sfm||e.fmt||t.name,category_id:`molluscs`,category_name:`Fossile Certifié`,color:`#0284c7`,precision_gps:`📍 Point GPS Certifié PBDB`,source:`PBDB`})))})}))},document.addEventListener(`DOMContentLoaded`,()=>{t(),u(),A(),ee(),W(),fetch(`processed/departments.json`).then(e=>e.json()).then(e=>{let t=document.getElementById(`deptSelect`);t&&(t.innerHTML=e.map(e=>`<option value="${e.code}">${e.code} — ${e.name}</option>`).join(``),e.forEach(e=>{$[e.code]=e}),t.value=`34`)}),fetch(`processed/34/fossils.json`).then(e=>e.json()).then(e=>d(e)),fetch(`processed/herault_geologie_pentes.geojson`).then(e=>e.json()).then(e=>x(e)),fetch(`/api/seasonality`).then(e=>e.json()).then(e=>{if(e&&e.mois){let t=document.getElementById(`season-month`),n=document.getElementById(`season-score`),r=document.getElementById(`season-condition`),i=document.getElementById(`season-advice`);t&&(t.innerText=e.mois),n&&(n.innerText=e.score_saison+` / 100`,e.score_saison>=80?n.style.color=`#34d399`:e.score_saison>=60?n.style.color=`#f59e0b`:n.style.color=`#ef4444`),r&&(r.innerText=e.condition),i&&(i.innerText=e.conseil)}}).catch(()=>{})});